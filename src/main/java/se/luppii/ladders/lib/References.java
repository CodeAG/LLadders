package se.luppii.ladders.lib;

public class References {

	public static final String MOD_ID = "LLadders";

	public static final String MOD_NAME = "Luppii's Ladders";

	public static final String MOD_STRING = "lladders.";

	public static final String MOD_VERSION = "0.4.0";

	public static final String CLIENT_PROXY_LOCATION = "se.luppii.ladders.proxy.ClientProxy";

	public static final String COMMON_PROXY_LOCATION = "se.luppii.ladders.proxy.CommonProxy";

	// Helper mod for Ropes+ mod
	public static final String MOD_HELP_ROPES_ID = "LLadders|RopesPlus";

	public static final String MOD_HELP_ROPES_NAME = "Luppii's Ladders Compatible: Ropes+";

	public static final String MOD_HELP_ROPES_VERSION = "0.0.1";
	
	// Helper mod for Thaumcraft mod
	public static final String MOD_HELP_THAUMCRAFT_ID = "LLadders|Thaumcraft";

	public static final String MOD_HELP_THAUMCRAFT_NAME = "Luppii's Ladders Compatible: Thaumcraft 4";

	public static final String MOD_HELP_THAUMCRAFT_VERSION = "0.0.1";
}
